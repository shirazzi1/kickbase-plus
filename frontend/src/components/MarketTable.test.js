...
jest.mock("../data/market.json", () => {
    const row = (overrides) => ({
        ...
        expiration: null,
        avgDailyGrowth: 500,
        // Non-null by default so the delta columns never render their own "–", which
        // would otherwise collide with the one this suite cares about (the "Dein Gebot"
        // column after a withdrawal)
        today: 1000,
        yesterday: 1000,
        twoDays: 1000,
        sevenDaysAvg: 1000,
